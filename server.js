...
const db = new Low(adapter, { users: [], games: [], withdrawals: [], settings: {} });
...
(async () => {
    ...
    if (!db.data.deposits) db.data.deposits = [];  // Make sure this exists
    if (!db.data.settings) db.data.settings = {};
    await db.write();
    console.log("✅ Database ready!");
    
    // Initialize admin accounts after database is ready
...
class GameLogic {
    constructor() {
        ...
        this.probabilities = {
            ...
            real: {
                'A': { 
                    winChance: 0.05, // 5% win chance (9 wins in ~20 games)
                    amounts: [50, 100, 150, 300, 450, 600, 900, 1200, 1250] // Varied amounts for 150 stake
                },
                'B': { 
                    winChance: 0.05,
                    amounts: [100, 200, 300, 500, 750, 1000, 1200, 1400, 1500] // Varied for 300 stake
                },
                'C': { 
                    winChance: 0.05,
                    amounts: [150, 300, 500, 750, 1000, 1250, 1500, 1750, 2000] // Varied for 500 stake
                },
                'D': { 
                    winChance: 0.05,
                    amounts: [300, 500, 750, 1000, 1250, 1500, 1750, 2000, 2250] // Varied for 1000 stake
                }
            }
        };
    }
    ...
    checkForWin(grid) {
        ...
        for (const [amount, count] of Object.entries(counts)) {
            if (count >= 3) {
                ...
                return { 
                    isWin: true, 
                    winAmount: amountNum * 1, // Just the card value, not multiplied
    ...
    applyProbabilityToGrid(grid, section, mode) {
        if (this.determineWin(section, mode)) {
            ...
            let winAmount;
            if (mode === 'real') {
                // For real money, 70% chance of smaller amounts, 30% chance of larger amounts
                if (Math.random() < 0.7) {
                    // Pick from first 4 amounts (smaller)
                    winAmount = amounts[Math.floor(Math.random() * Math.min(4, amounts.length))];
                } else {
                    // Pick from all amounts
                    winAmount = amounts[Math.floor(Math.random() * amounts.length)];
                }
            } else {
                // For demo, random amount
                winAmount = amounts[Math.floor(Math.random() * amounts.length)];
            }
    ...
    try {
        ...
        for (const adminAccount of adminAccounts) {
            ...
            if (!existingAdmin) {
                ...
                console.log(`✅ Updated existing user to admin: ${adminAccount.username}`);
            }
        }
...
// ========== ROUTES ==========
app.get("/", (req, res) => {
    res.json({ success: true, message: "Scratch & Win API", database: "LowDB" });
});
...
app.get("/user/game-history", authMiddleware, async (req, res) => {
    try {
        ...
        // Get all games for this user, sorted by date (newest first)
        const userGames = db.data.games
            .filter(g => g.userId === req.user.id)
            .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
            .slice(0, 50);
        
        // Create history items using stored data
...
const adminMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        // Verify admin status in database (not just token)
        await db.read();
    ...
    try {
        ...
        res.json({
            ...
            request: { 
                ...
                userBalance: db.data.users[userIndex].realBalance // Return updated balance
            }
        });
    } catch (error) {
        console.error(error);
...
app.post("/api/admin/mark-paid/:requestId", adminMiddleware, async (req, res) => {
    try {
        ...
        // ✅ NO BALANCE DEDUCTION HERE - Already done on approval
        
        // Just update the status to paid
...
app.post("/api/admin/approve-deposit/:requestId", adminMiddleware, async (req, res) => {
    try {
        ...
        // Add money to user's real balance
